...
get_data_interval = 20
ramp_lanes_num = 4
merge_area_lane = 5
discrete_mainstream_arriving_flow = 100
discrete_density_of_merging_area = 2
discrete_ramp_queue = 10
discrete_red_duration = 2
...
for i in range(1):
    ...
    # 存储数据
    queue.extend(q[int(warmup/get_data_interval):])
    redtime.extend(r[int(warmup/get_data_interval):])
    # 获取输出文件中的检测器数据
    ...
    mainline_flow_array = np.vstack((np.array(flow['814 SB']) / 3, np.array(flow['813 SB']) / 3, np.array(flow['812 SB']) / 5,
                                     np.array(flow['811 SB']) / 4, np.array(flow['810 SB']) / 3)).T * 3600 / get_data_interval
    ...
    ramp_flow_array = (np.array(flow['ramp']).T * 3600 / 20 / 2).reshape(-1, 1)
    ramp_speed_array = (np.array(speed['ramp']).T * 3.6).reshape(-1, 1)
    rl_state_array1 = ((np.array(flow['814 SB'])/3 + np.array(flow['813 SB'])/3) * 3600 / get_data_interval / 2 / discrete_mainstream_arriving_flow).astype(
        int).reshape(-1, 1)
    rl_state_array2 = np.where(np.array(flow['812 SB']) != 0,
                               np.array(flow['812 SB']) * 3600 / get_data_interval / 5 / (np.array(speed['812 SB']) * 3.6) / discrete_density_of_merging_area,
                               0).astype(int).reshape(-1, 1)
    rl_state_array3 = (ramp_queue_array / discrete_ramp_queue).astype(int).reshape(-1, 1)
    rl_state_array4 = (np.array(redtime) / discrete_red_duration).astype(int).reshape(-1, 1)
    rl_action_array = (np.array(redtime) / discrete_red_duration).astype(int).reshape(-1, 1)  # Need to be moved one step forward
    ...
    rl_reward_array = np.array(flow['810 SB']).T.reshape(-1,
                                                         1) * 3600 / 20 / 3 * rl_queue_punish  # Need to be moved one step forward
